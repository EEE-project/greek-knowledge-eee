...
class WiktextractIndex:
    ...
    @classmethod
    def load(cls, jsonl_path: Path) -> "WiktextractIndex":
        ...
        per word — the same word can recur across multiple lines (e.g.
        distinct POS or etymologies). This index keeps only the last
        entry seen for a given word, matching lookup()'s single-dict
        return type; earlier entries for the same word are discarded."""
