---
type: Grammatical Rule
title: demonstratives-and-indefinites
description: αυτός/εκείνος + article + noun (the article is obligatory), the -ν forms αυτόν/αυτήν, and κανένας in questions.
tags:
- pronoun
- demonstrative
- indefinite
- article
- modern-greek
level:
- A2
sources:
- id: livexp-grammar
  resource: /home/sadov/work/greek/lectures/LiveXP/Γραμμστική.md
  title: LiveXP Modern Greek grammar notes
  author: LiveXP course notes
- id: ellinika-a
  resource: Ελληνικά Α - Επίπεδα Α1-Α2 - 2015.pdf, Ενότητες 9, 16, 19
  title: 'Ελληνικά Α΄: Μέθοδος εκμάθησης της ελληνικής ως ξένης γλώσσας, Επίπεδα Α1-Α2 (Εκδόσεις Πατάκη, 2010)'
  author: Γιώργος Σιμόπουλος, Ειρήνη Παθιάκη, Ρίτα Κανελλοπούλου, Αγλαΐα Παυλοπούλου
generated:
  by: human
  at: '2026-09-21T07:42:00+00:00'
periods_spanned:
  from: modern
  to: modern
dialect: []
status: draft
verified: []
---
## "This" and "that" keep the article: αυτός ο υπολογιστής

**αυτός, αυτή, αυτό** mean "he/she/it" as personal pronouns but "this" when they stand before a noun, and in that use the noun keeps its definite article: **αυτό το πουκάμισο** "this shirt," **αυτά τα κορίτσια** "these girls." The article is obligatory, and the same holds for **εκείνος, εκείνη, εκείνο** "that."[^livexp-grammar] The textbook shows the three genders and the plural side by side -- **Αυτός ο υπολογιστής είναι ακριβός.**, **Αυτή η μηχανή είναι ακριβή.**, **Αυτό το κινητό είναι ακριβό.**, **Αυτοί οι υπολογιστές είναι ακριβοί.** -- and pairs the two demonstratives for contrast: **Αυτός ο υπολογιστής είναι ακριβός.** / **Εκείνος ο υπολογιστής είναι φτηνός.**[^ellinika-a]

In the accusative singular the pronoun shows the same final **-ν** as the article that follows it: **αυτόν τον σκύλο**, **αυτόν τον άντρα** (LiveXP's mnemonic: **τον/την** after **αυτός/αυτή** turn the pronoun into **αυτόν/αυτήν**); the neuter is unchanged (**αυτό το ποδήλατο**), and a nominative subject uses the plain forms -- **Δεν μου αρέσει αυτός ο σκύλος.** "I don't like this dog" beside **Δεν αγαπάω αυτόν τον σκύλο.** "I don't love this dog."[^livexp-grammar] The textbook has the same pattern in **με αυτόν τον τρόπο** "in this way," and shows the feminine, where both words drop the **-ν** before **δ**, in **Συμπληρώστε αυτή τη δήλωση.** "Fill in this statement." (for when the **-ν** is written, see [definite-articles-nom-acc](definite-articles-nom-acc.md)).[^ellinika-a]

## κανένας in a question means "some/any"

**κανένας, καμία/καμιά, κανένα** is already introduced in Unit 9; in a question it stands for "some/any," as in the textbook's suggestions **Πάμε να πιούμε κανέναν καφέ;** "Shall we go for a coffee?" and **Θέλετε να δούμε καμιά ταινία;** "Would you like us to see a film?" -- see [kanenas-tipota](kanenas-tipota.md) for the fuller picture, including its negative reading.[^ellinika-a]

[^livexp-grammar]: LiveXP Modern Greek grammar notes, LiveXP course notes (/home/sadov/work/greek/lectures/LiveXP/Γραμμστική.md)
[^ellinika-a]: Ελληνικά Α΄: Μέθοδος εκμάθησης της ελληνικής ως ξένης γλώσσας, Επίπεδα Α1-Α2 (Εκδόσεις Πατάκη, 2010), Γιώργος Σιμόπουλος, Ειρήνη Παθιάκη, Ρίτα Κανελλοπούλου, Αγλαΐα Παυλοπούλου (Ελληνικά Α - Επίπεδα Α1-Α2 - 2015.pdf, Ενότητες 9, 16, 19)
